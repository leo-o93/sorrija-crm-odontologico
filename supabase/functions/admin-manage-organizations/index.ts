  ...
  try {
    ...
    if (req.method === 'POST' && pathParts.length === 0) {
      ...
      const { data: settings } = await supabase
        .from('admin_settings')
        .select('allow_new_organizations')
        .order('created_at', { ascending: true })
        .limit(1)
        .maybeSingle();

      if (settings && settings.allow_new_organizations === false) {
        return new Response(JSON.stringify({ error: 'Organization creation is disabled' }), {
          status: 403,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
    ...
    if (req.method === 'PUT' && pathParts.length === 3 && pathParts[1] === 'members') {
      const organizationId = pathParts[0];
      ...
      await logAudit({
        ...
        targetId: data.id,
        details: { organizationId, userId, role, active },
        req,
      });
